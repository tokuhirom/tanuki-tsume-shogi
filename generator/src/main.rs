...
fn parse_args() -> (u32, u32, u32, u64) {
    ...
    (max, attempts3, attempts5, seed)
}

fn main() {
    let (max, attempts3, attempts5, seed) = parse_args();

    let curated = generate::load_curated("data/curated-puzzles.json");

    for mate_len in [3u32, 5] {
        let attempts = if mate_len == 3 { attempts3 } else { attempts5 };
